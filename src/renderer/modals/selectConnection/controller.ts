...
${sortingUtilities}
    ...
    const renderConnections = (connectionsData, options = {}) => {
        ...
        if (useGroups) {
            connectionsListContainer.innerHTML = groupKeys.map(groupKey => {
            ...
            connectionsListContainer.querySelectorAll('.connection-group-header').forEach(header => {
                const toggleGroup = () => {
            ...
            connectionsListContainer.innerHTML = connections.map(renderConnectionItem).join('');
        }
