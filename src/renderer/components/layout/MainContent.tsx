...
export const MainContent: React.FC = () => {
    ...
    const { openTools, activeToolId } = useToolsContext();
                    ...
                    <div className="tool-panel-header">
                        <div className="tool-tabs" id="tool-tabs">
                            {Array.from(openTools.entries()).map(([toolId, openTool]) => (
                                <div key={toolId} className={`tool-tab ${activeToolId === toolId ? "active" : ""}`}>
                                    <span className="tool-tab-name">{openTool.tool.name}</span>
                                    <button className="tool-tab-close" onClick={() => {}} title="Close">
                                        ✕
                                    </button>
                                </div>
                            ))}
                        ...
                        <div className="tool-panel-controls">
                            <button className="btn-icon" title="Close all tools">
                                ✕
                            </button>
                        </div>
                    </div>
