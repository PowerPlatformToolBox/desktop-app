    ...
    "configuration": [
      {
        ...
        "properties": {
          "myTool.enabled": {
            "type": "boolean",
            "default": true,
            "description": "Enable or disable my tool"
          }
        }
      }
    ]
...
Contribution points define what your tool adds to the ToolBox. They are declared in the `contributes` section of `package.json`.

### Commands

Commands are actions that users can invoke:
...
Define custom UI panels (future feature):
...
Control when your tool is loaded:

```json
{
  "activationEvents": [
    "onCommand:myTool.command1",  // When command is executed
    "onView:myTool.view1",         // When view is opened
    "*"                             // On ToolBox startup
  ]
}
```
...
- `onCommand:commandId` - When a specific command is invoked
- `onView:viewId` - When a specific view is opened
- `*` - On ToolBox startup (use sparingly)
