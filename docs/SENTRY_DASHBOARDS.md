...
- **Install ID Tagging**: All events tagged with install ID for per-installation analysis
...
- **How to create**:
    1. Click **Add Widget** → **Discover**
    2. Set **Display**: `tags.install_id`, `avg(duration)` if available
    3. Set **Group by**: `tags.install_id`
    4. Set **Conditions**: `age:-7d`
    5. Set **Sort by**: duration descending
    6. Set **Limit**: 10
    7. Save
- **Display**: Table with install IDs and average operation duration
- **Columns**: Install ID, Operation count, Duration
- **Position**: Bottom, full width
- **Purpose**: Identify installations with performance issues
- **Action**:
...
- **How to create**:
    1. Click **Add Widget** → **Discover**
    2. Set **Display**: `count(unique tags.install_id)` (count unique installs)
    3. Set **Group by**: `geo.country_code`
...
#### Investigating a Specific Installation

1. Note the `install_id` from the issue
2. Go to **Discover**
3. Create new query with filter: `tags.install_id:"[install_id]"`
4. View all issues from that installation
5. Look at breadcrumbs and stack traces
6. Check if the install is in a high-error region (use Dashboard 4)
7. Contact user for support or diagnostics

#### Resolution Tracking

- Tag issue with install ID and region
- Document resolution steps
- Monitor installation after fix (should see error drop)
