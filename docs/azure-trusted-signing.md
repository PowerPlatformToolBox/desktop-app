...
This document explains how Windows artifacts generated by the production release pipeline are signed with [Azure Trusted Signing](https://learn.microsoft.com/azure/artifact-signing/overview) without exposing private keys.
...
- The job running on `windows-latest` now executes the [`azure/artifact-signing-action@v1`](https://github.com/Azure/artifact-signing-action) step immediately after Electron Builder packages the installers.
- Every `.exe` and `.msi` produced under `build/` is signed in place. The action handles hashing, timestamping, and certificate management by requesting a short-lived certificate from Azure Trusted Signing.
...
2. Trigger the `Stable Release` workflow via the `workflow_dispatch` entry point (you can do this on a throwaway branch after updating the version and release notes).
