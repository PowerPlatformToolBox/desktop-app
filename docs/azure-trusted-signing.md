...
- Every `.exe` and `.msi` produced under `build/` is signed in place. The action handles hashing, timestamping, and certificate management by requesting a short-lived certificate from Azure Trusted Signing before any `.zip` archives are created.
